...
func TestNormalizeSorts(t *testing.T) {
	...
	t.Run("deduplicate by normalized column", func(t *testing.T) {
		sorts := []*basev1.Sort{
			{Field: "name", Order: basev1.SortOrder_SORT_ORDER_ASC},
			{Field: "name", Order: basev1.SortOrder_SORT_ORDER_DESC},
			{Field: "updatedAt", Order: basev1.SortOrder_SORT_ORDER_DESC},
		}
	...
	t.Run("invalid field", func(t *testing.T) {
		_, err := utils.NormalizeSorts([]*basev1.Sort{{Field: "bad_field"}}, cfg, 5)
	...
	t.Run("max sort limit", func(t *testing.T) {
		sorts := []*basev1.Sort{{Field: "name"}, {Field: "updated_at"}}
...
func TestNormalizeFilters(t *testing.T) {
	...
	t.Run("text and uuid filters", func(t *testing.T) {
		filters := []*basev1.Filter{
			{Field: "name", Operator: basev1.FilterOperator_FILTER_OPERATOR_LIKE, StringValue: strPtr("abc")},
			{Field: "productId", Operator: basev1.FilterOperator_FILTER_OPERATOR_IN, StringValues: []string{id1, id2}},
		}
	...
	t.Run("invalid operator for field", func(t *testing.T) {
		filters := []*basev1.Filter{{Field: "description", Operator: basev1.FilterOperator_FILTER_OPERATOR_IN, StringValues: []string{"x"}}}
	...
	t.Run("invalid uuid value", func(t *testing.T) {
		filters := []*basev1.Filter{{Field: "product_id", Operator: basev1.FilterOperator_FILTER_OPERATOR_EQUAL, StringValue: strPtr("not-uuid")}}
		_, err := utils.NormalizeFilters(filters, cfg, 10)
	...
	t.Run("max filter limit", func(t *testing.T) {
		filters := []*basev1.Filter{{Field: "name", StringValue: strPtr("a")}, {Field: "name", StringValue: strPtr("b")}}
...
func TestBuildListAndCountQueries(t *testing.T) {
	...
	filters := []utils.FilterClause{{Column: "name", DataType: utils.FieldDataTypeText, Operator: basev1.FilterOperator_FILTER_OPERATOR_EQUAL, Value: "demo"}}
...
func TestParseFilterValueTimestamp(t *testing.T) {
	...
	out, err := utils.ParseFilterValue(
		&basev1.Filter{Field: "created_at", StringValue: &v},
		utils.FieldDataTypeTimestamp,
		basev1.FilterOperator_FILTER_OPERATOR_EQUAL,
	)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
